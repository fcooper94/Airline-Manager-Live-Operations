...
function renderFlightBlocks(flights) {
  ...
  return flights.map(flight => {
    ...
    const returnDepTime = returnDepDate.toTimeString().substring(0, 5);
    const returnArrTime = returnArrDate.toTimeString().substring(0, 5);
    ...
    return `
      <div
        ...
        style="
          ...
          top: 0;
          left: ${leftPercent}%;
          width: calc(${widthPercent}% + ${Math.floor(widthPercent / 100)}*50px);
          height: 100%;
          min-height: 50px;
        ...
        title="${route.routeNumber}/${route.returnRouteNumber}: ${depAirport}→${arrAirport}→${depAirport} | Dep ${depTime} Arr ${returnArrTime}"
      >
        ...
        <div style="grid-column: 1 / 2; grid-row: 2; text-align: left; font-size: 0.6rem; opacity: 0.85;">${returnDepTime}</div>
...
function renderSchedule() {
  ...
  timeColumns.forEach(col => {
    html += `<th style="padding: 0.75rem 0.25rem; text-align: center; color: var(--text-secondary); font-weight: 600; min-width: 50px; border-left: 1px solid var(--border-color);">${col.label}</th>`;
  });
  ...
  timeColumns.forEach((col, index) => {
    ...
    html += `
      <td
        ...
        style="padding: 0.5rem 0.25rem; text-align: center; background: var(--surface-elevated); ${borderStyle} min-height: 70px; min-width: ${cellWidth}; position: relative; vertical-align: top;"
      >
...
function handleDragOver(event) {
  ...
  preview.textContent = `${draggedRoute.routeNumber} / ${draggedRoute.returnRouteNumber} @ ${routeTime}`;
}
...
async function handleDrop(event, aircraftId, timeValue) {
  ...
  // Clear all drag-over highlighting from the row
  const row = cell.closest('tr');
  if (row) {
    row.querySelectorAll('.schedule-cell').forEach(c => c.classList.remove('drag-over'));
  }
...
async function fetchWorldTime() {
  try {
    ...
    if (response.ok) {
      ...
      scheduleWorldReferenceTime = new Date(worldInfo.currentTime);
      scheduleWorldReferenceTimestamp = Date.now();
      scheduleWorldTimeAcceleration = worldInfo.timeAcceleration || 60;

      console.log('World time:', scheduleWorldReferenceTime, 'Day of week:', scheduleWorldReferenceTime.getDay());
...
function getCurrentWorldTime() {
  if (!scheduleWorldReferenceTime || !scheduleWorldReferenceTimestamp) return null;
  ...
  return new Date(scheduleWorldReferenceTime.getTime() + gameElapsedMs);
}
...
function updateTimeline() {
  ...
  // Calculate position with seconds for smooth movement
  const hours = currentTime.getHours();
  const minutes = currentTime.getMinutes();
  const seconds = currentTime.getSeconds();

  // Calculate total time in hours (with decimal precision for smooth movement)
  const totalHours = hours + (minutes / 60) + (seconds / 3600);

  // Calculate position: we have 24 columns after the first (aircraft) column
  // First column (aircraft info) is ~200px
  // Each time column is ~50px
  const firstColumnWidth = 200;
  const columnWidth = 50;
  const position = firstColumnWidth + (totalHours * columnWidth);
